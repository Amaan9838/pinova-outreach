...
export class CampaignValidationService {
  ...
  static async validateMailbox(campaign) {
    ...
    // Check if mailbox is selected — accept options.selectedMailbox, campaign.mailbox, or mailboxIds[]
    ...
    if (!selectedMailbox) {
      errors.push({
        ...
        message: 'No mailbox selected. Set one in the Options tab and save.'
      });
    ...
    // Check daily limits — query actual Messages sent today (source of truth)
    // This replaces the stale dailySent counter which never auto-resets.
    const Message = (await import('../models/Message.js')).default;
    const todayStart = new Date();
    todayStart.setUTCHours(0, 0, 0, 0);
    const todaySent = await Message.countDocuments({
      mailboxId: mailbox._id,
      createdAt: { $gte: todayStart },
      status: { $nin: ['queued', 'failed'] }
    });
    
    if (todaySent >= mailbox.dailyCap) {
      errors.push({
        code: 'MAILBOX_DAILY_LIMIT_REACHED',
        message: `Mailbox ${mailbox.fromEmail} has reached daily sending limit (${todaySent}/${mailbox.dailyCap} sent today)`
      });
    }
 ...
 const warnings = []; // New: Non-blocking issues
  const fixActions = []; // New: Quick fixes (e.g., { code: 'INIT_SCHEDULE', action: 'Call scheduleNextStep' })
    ...
    try {
      ...
      if (campaignProspectCount > 0) {
        ...
        // Validate scheduling readiness — only for legacy (non-v2) campaigns
        // v2 uses nextActionAt (not nextSendAt), managed entirely by outreachEngine
        if (!campaign.useV2Engine && (campaign.status === 'active' || campaign.status === 'scheduled')) {
          ...
          const overdueProspects = campaignProspects.filter(cp =>
            cp.status === 'active' &&
            cp.nextSendAt &&
            new Date(cp.nextSendAt) < new Date(Date.now() - 60 * 60 * 1000)
          );

          if (overdueProspects.length > 0) {
            errors.push({
              code: 'OVERDUE_PROSPECTS',
              message: `${overdueProspects.length} prospects have overdue send times`
            });
          }
        }
  ...
  static validateSequence(campaign) {
    ...
    // ── v2 Engine or Flow Builder: no sequence steps required ────────────────
    // v2 generates email content via AI (or per-lead emailSteps). Skip entirely.
    if (campaign.useV2Engine || campaign.useVisualFlow || campaign.emailFlow) {
      console.log(`[validation] Skipping sequence check — useV2Engine=${campaign.useV2Engine}, useVisualFlow=${campaign.useVisualFlow}`);
    ...
    if (!campaign.sequence || campaign.sequence.length === 0) {
      errors.push({
        ...
        message: 'Campaign must have at least one sequence step (or use the Flow Builder)'
      });
      return { valid: false, errors };
    }
