...
export class CampaignValidationService {
  ...
  static async validateMailbox(campaign) {
    ...
    // Check daily limits — auto-reset counter if the day has changed
    const freshMailbox = await Mailbox.resetDailySentIfNeeded(mailbox._id);
    const todaySent = freshMailbox?.dailySent ?? mailbox.dailySent;
    if (todaySent >= mailbox.dailyCap) {
      errors.push({
        code: 'MAILBOX_DAILY_LIMIT_REACHED',
        message: `Mailbox ${mailbox.fromEmail} has reached daily sending limit (${todaySent}/${mailbox.dailyCap} today)`
      });
    }
