...
function hasUnprocessedReply(lead) {
  if (!lead.repliedAt) return false;
  if (!lead.replyCategorizedAt) return true;

  return new Date(lead.repliedAt).getTime() > new Date(lead.replyCategorizedAt).getTime();
}

function normalizeReplyCategory(intent) {
  if (intent === 'positive') return 'positive';
  if (intent === 'objection') return 'objection';
  if (intent === 'stop') return 'unsubscribe';
  return 'neutral';
}
...
export async function processLead(leadId) {
  ...
  try {
    ...
    const allowedDays = getAllowedBusinessDays(campaign);
    const enforceBusinessWindow = shouldEnforceBusinessHours(campaign);
    const hasPendingReply = hasUnprocessedReply(lead);
    ...
    if (hasPendingReply) {
      ...
      if (intent === 'positive') {
        ...
        await CampaignProspect.findByIdAndUpdate(leadId, {
          ...
          nextActionAt: null,
          processingLock: false,
          replyCategory: normalizeReplyCategory(intent),
        ...
        if (replyEmail?.subject && replyEmail?.body) {
          ...
          try {
            replySendResult = await SMTPService.sendEmail({
              ...
              html: `<div dir="ltr">${replyEmail.body.replace(/\n/g, '<br>')}</div>`,
              ...
              disableTracking: !campaign.options?.trackOpens
            });
          } catch (smtpErr) {
          ...
          if (replySendResult?.success) {
            ...
            // Once a prospect replies, the normal campaign sequence stays stopped.
      ...
      await CampaignProspect.findByIdAndUpdate(leadId, {
        ...
        processingLock: false,
        replyCategory: normalizeReplyCategory(intent),
        replyCategoryConfidence: classification.confidence ?? null,
        replyCategorizedAt: new Date(),
        ...aiMemoryUpdate
      });
      await writeLog({
    ...
    if (lead.v2State?.startsWith('replied_')) {
      await CampaignProspect.findByIdAndUpdate(leadId, {
        nextActionAt: null,
        processingLock: false
      });
    ...
    // ── CLICK TRACKING — DISABLED FOR DELIVERABILITY ─────────────────────────
    // Wrapping URLs through redirect chains (vercel.app/api/track/click/...?url=...)
    // triggers phishing detection in Gmail/Outlook. Every link becomes a redirect
    // through a shared platform domain — the exact signature of phishing emails.
    // This was the #1 cause of 100% spam rate.
    //
    // When sending from Gmail web, URLs are raw. We match that behavior here.
    // TODO: Re-enable when a custom tracking domain (e.g. trk.yourdomain.com)
    // is configured so the link domain matches the sender domain.
    ...
    try {
      sendResult = await SMTPService.sendEmail({
        ...
        subject,
        html: `<div dir="ltr">${trackedHtml}</div>`,
        text: trackedBody,
        trackingId: generatedTrackingId,
        inReplyTo,
        references,
