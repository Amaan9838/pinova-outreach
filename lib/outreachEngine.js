...
export function calculateNextActionAt(campaign, attemptCount, openStatus = false) {
  ...
  if (attemptCount === 0) {
    ...
    // Return next valid business hour immediately (don't add delay)
    return enforceBusinessHours(new Date(), timezone, startHour, endHour, allowedDays);
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// §7.3 — Rate limiting check for mailbox
...
async function checkMailboxRateLimits(campaign, mailbox) {
  ...
  // Check minimum gap (PRD §3.10)
  if (mailbox.lastSentAt) {
    const gapMs = now - new Date(mailbox.lastSentAt);
    const minGapMs = (limits.minGapMinutes || 3) * 60 * 1000;
    if (gapMs < minGapMs) {
      const waitMs = minGapMs - gapMs;
      console.log(`[v2Engine] Min-gap limit: waiting ${Math.round(waitMs / 1000)}s`);
      return addMinutes(now, Math.ceil(waitMs / 60000));
    }
  }
...
export async function processLead(leadId) {
  ...
  try {
    ...
    // Mailbox lookup: try campaign.mailbox first, then mailboxIds[], then options.selectedMailbox
    const Mailbox = (await import('../models/MailboxFixed.js')).default;
    const mailboxId =
      campaign.mailbox ||
      (campaign.mailboxIds?.length ? campaign.mailboxIds[0] : null) ||
      campaign.options?.selectedMailbox;
    ...
    if (!mailbox || mailbox.status !== 'active') {
      await releaseLead(leadId, lead.nextActionAt);
      await writeLog({ ...logData, stateAfter: stateBefore || 'new', action: 'skipped_mailbox_inactive', error: `Mailbox ${mailboxId} inactive or not found` });
      console.warn(`[v2Engine] Mailbox ${mailboxId} not active. Skipping lead ${leadId}.`);
      return;
    }
    ...
    if (isProd && enforceBusinessWindow) {
    ...
    if (lead.repliedAt && !lead.v2State?.startsWith('replied_')) {
      ...
      if (intent === 'positive') {
        ...
        nextActionAt = calculateNextActionAt(campaign, lead.attemptCount);
      ...
      if (intent === 'positive' || intent === 'objection') {
        let replyEmail;
        try {
        ...
        if (replyEmail?.subject && replyEmail?.body) {
          ...
          if (replySendResult?.success) {
            ...
            // Reply was sent — move to a settled state, schedule next action normally
            newState = intent === 'positive' ? 'replied_positive' : 'replied_objection';
            nextActionAt = calculateNextActionAt(campaign, (lead.attemptCount || 0) + 1);
            await writeLog({
              ...logData,
              stateAfter: newState,
      ...
      await CampaignProspect.findByIdAndUpdate(leadId, {
        v2State: newState, nextActionAt, processingLock: false, ...aiMemoryUpdate
      });
      await writeLog({
    ...
    if (stepContent?.subject && stepContent?.body) {
      ...
      let generated;
      try {
        generated = await generateTargetedEmail({
          campaignGoal: campaign.goal,
          knowledgeBase: campaign.knowledgeBase || '',
          lead: {
            name: lead.prospect?.name || `${lead.prospect?.firstName || ''} ${lead.prospect?.lastName || ''}`.trim() || '',
            email: lead.prospect?.email || '',
            company: lead.prospect?.company || '',
            additionalContext: lead.personalizedData ? JSON.stringify(lead.personalizedData) : ''
          },
          selectedAngle,
          escalationLevel,
          ...
          previousMessages,
          openStatus,
          replyContext: lead.aiMemory?.replySummary ? {
            summary: lead.aiMemory.replySummary,
            objectionType: lead.aiMemory.objectionType
          } : undefined
        });
      } catch (aiErr) {
        // Retry once (PRD §6.11)
        try {
          generated = await generateTargetedEmail({
            campaignGoal: campaign.goal,
            knowledgeBase: campaign.knowledgeBase || '',
            lead: { name: lead.prospect?.firstName || '', email: lead.prospect?.email || '' },
            selectedAngle, escalationLevel,
            attemptCount: attemptCount + 1,
            previousMessages, openStatus
          });
        } catch {
          const newFailureCount = (lead.failureCount || 0) + 1;
          const nextAt = newFailureCount > 5 ? null : calculateRetryBackoff(newFailureCount);
          const newState = newFailureCount > 5 ? 'failed' : (stateBefore || 'new');
          await CampaignProspect.findByIdAndUpdate(leadId, {
            failureCount: newFailureCount,
            nextActionAt: nextAt,
            v2State: newState,
            stopFlag: newFailureCount > 5,
            processingLock: false
          });
          await writeLog({ ...logData, stateAfter: newState, action: 'retry_scheduled', error: 'AI generation failed twice', errorCategory: 'ai_generation_failure', nextActionAtAfter: nextAt, failureCount: newFailureCount });
          return;
        }
      }
    ...
    const inReplyTo = lead.threadHeaderMessageId || null;
    ...
    const trackedHtml = trackedBody.replace(/\n/g, '<br>');
    ...
    if (sendResult.success) {
      ...
      // Set threadHeaderMessageId on first send (PRD §7.3)
      const threadUpdate = isInitialSend && sendResult.headerMessageId
        ? { threadHeaderMessageId: sendResult.headerMessageId }
        : {};
