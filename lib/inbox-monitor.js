...
export class InboxMonitorService {
  static async checkReplies() {
    ...
    const activeMailboxes = await Mailbox.find({ 
      status: 'active',
      imapConfiguration: { $exists: true }
    });

    console.log(`Found ${activeMailboxes.length} active mailboxes with IMAP`);
  ...
  static async handleReply(originalMessage, parsedEmail) {
    try {
      ...
      // Add reply event to original message
      originalMessage.events.push({
        ...
        data: {
          ...
          content: parsedEmail.text?.substring(0, 500) // First 500 chars
        }
      });
