...
export class InboxMonitorService {
  ...
  static async sendTemplateAutoReply(mailbox, originalMessage, parsedEmail, inboundReplyContent) {
    ...
    const campaignTemplate = originalMessage.campaignId?.autoReplyTemplate;
    ...
    const baseThreadSubject = originalMessage.subject || parsedEmail.subject || 'Website preview';
    const subjectTemplate = campaignTemplate?.subject || process.env.AUTO_REPLY_TEMPLATE_SUBJECT || (/^re:/i.test(baseThreadSubject) ? baseThreadSubject : `Re: ${baseThreadSubject}`);
    ...
    const references = Array.isArray(originalMessage.references)
      ...
      : [];
    if (originalMessage.headerMessageId) references.push(originalMessage.headerMessageId);
