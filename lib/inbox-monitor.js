...
export class InboxMonitorService {
    ...
    return new Promise((resolve, reject) => {
      ...
      imap.once('ready', () => {
        imap.openBox('INBOX', false, (err, box) => {
          ...
          imap.search(criteria, (err, searchResults) => {
            if (err) {
            ...
            if (mailbox.lastProcessedUid && mailbox.lastProcessedUid > 0) {
              results = results.filter(uid => uid > mailbox.lastProcessedUid);
            }
  ...
  static async processReply(mailbox, parsedEmail) {
    try {
      ...
      const alreadyRecorded = originalMessage.events?.some(e => {
        if (e.type !== 'replied') return false;
        const msgId = typeof e.data?.get === 'function' ? e.data.get('messageId') : e.data?.messageId;
        const hash = typeof e.data?.get === 'function' ? e.data.get('replyHash') : e.data?.replyHash;
        return msgId === parsedEmail.messageId || hash === replyHash;
      });
      if (alreadyRecorded) {
  ...
  static async sendTemplateAutoReply(mailbox, originalMessage, parsedEmail, inboundReplyContent) {
    ...
    const prospectId = prospect?._id || originalMessage.prospectId;
    const alreadySentTemplateReply = await Message.exists({
      campaignId: campaignId || originalMessage.campaignId,
      prospectId,
      isResponse: true,
      trackingId: /template-reply/
    });

    if (alreadySentTemplateReply) {
      console.log(`Template auto-reply already sent to ${toEmail}; skipping repeat acknowledgement`);
      return;
    }
