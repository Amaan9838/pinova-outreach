...
export class InboxMonitorService {
  ...
  static async handleReply(originalMessage, parsedEmail, replyHash) {
    try {
      ...
      // CRITICAL FIX: Stop ALL scheduling systems atomically
  ...
  static async stopAllSchedulingForProspect(campaignId, prospectId) {
    ...
    try {
      // 1. Update campaign prospect status (legacy system)
      const campaignUpdate = await Campaign.updateOne(
        { 
          _id: campaignId,
      ...
      // 2. Cancel all pending scheduled emails (event-driven system)
      const scheduledEmailUpdate = await ScheduledEmail.updateMany(
        {
          campaignId,
          prospectId,
          status: 'pending'
        },
        {
          $set: { 
            ...
            lastError: 'Prospect replied - sequence stopped'
          }
        }
      );
      
      console.log(`Campaign prospect update: ${campaignUpdate.modifiedCount} records`);
      console.log(`Scheduled email cancellations: ${scheduledEmailUpdate.modifiedCount} records`);
      
      // 3. Update prospect status to prevent future scheduling
