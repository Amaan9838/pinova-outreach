...
export class InboxMonitorService {
  ...
  static async handleReply(originalMessage, parsedEmail, replyHash) {
    try {
      ...
      // ── Wire 5: Fire FlowEngine 'email_replied' event ─────────────────────────
      // This lets the flow evaluate 'replied?' condition nodes and route to:
      //   - a 'send_response' action (pre-written reply)
      //   - a 'categorize' node (route by intent)
      //   - an 'end' node (stop the sequence)
      try {
        ...
        if (cp?.campaign?.useVisualFlow) {
          const replyContent = parsedEmail.text || parsedEmail.html || '';
          console.log(`[InboxMonitor] Firing FlowEngine 'email_replied' for CampaignProspect ${cp._id}`);
          // Fire-and-forget — reply is already recorded above
          FlowEngine.executeTrigger(cp._id.toString(), 'email_replied', {
            replyContent,
            replyHash,
            fromEmail: parsedEmail.from?.value?.[0]?.address,
          }).catch(err => console.error('[InboxMonitor] FlowEngine error:', err.message));
        }
      } catch (feErr) {
        console.error('[InboxMonitor] Could not trigger FlowEngine:', feErr.message);
      }
