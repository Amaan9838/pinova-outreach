          ...
          case 'active':
            ...
            if (prospect.status === 'pending') {
              ...
              if (campaign && campaign.useV2Engine) {
                ...
                // §7.10 — Round-robin mailbox assignment from campaign pool
                ...
                if (mailboxPool.length > 0) {
                  setFields.assignedMailbox = mailboxPool[updates.length % mailboxPool.length];
                  console.log(`[V2] Assigned mailbox ${setFields.assignedMailbox} to prospect ${prospect._id} (pool index: ${updates.length % mailboxPool.length}/${mailboxPool.length})`);
                }
