        ...
        switch (campaignStatus) {
          case 'active':
            ...
            if (prospect.status === 'pending') {
              ...
              // Only set immediate send time if no nextSendAt exists
          ...
          case 'scheduled':
            // Campaign is scheduled - keep prospects as pending but set future send times
            if (prospect.status === 'pending' && options.startDateTime) {
              const staggerDelay = updates.length * 2 * 60 * 1000;

              // CRITICAL FIX: Use proper timezone conversion for scheduled times
              const campaignTimezone = options.timezone || 'UTC';
              let scheduledTime;

              if (campaignTimezone === 'UTC') {
                scheduledTime = new Date(options.startDateTime.getTime() + staggerDelay);
              } else {
                // Convert scheduled time from campaign timezone to UTC for database storage
                const utcStartTime = convertScheduledTimeToUTC(options.startDateTime, campaignTimezone);
                scheduledTime = new Date(utcStartTime.getTime() + staggerDelay);
              }

              setFields.nextSendAt = scheduledTime;
              console.log(`Scheduled prospect ${prospect._id} for ${scheduledTime.toISOString()} (campaign TZ: ${campaignTimezone})`);
            }
            break;
        }
