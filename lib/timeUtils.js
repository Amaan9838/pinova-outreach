...
export const convertScheduledTimeToUTC = (scheduledTime, campaignTimezone = 'UTC') => {
  ...
  const timeString = dateObj.toLocaleString('sv-SE'); // YYYY-MM-DD HH:mm:ss format
  const dateInCampaignTZ = new Date(timeString + ' UTC'); // Treat as UTC first

  // Get the offset for the campaign timezone
  const tempDate = new Date();
  const utcTime = tempDate.getTime() + (tempDate.getTimezoneOffset() * 60000);
  const campaignTime = new Date(utcTime + getTimezoneOffset(campaignTimezone));
  const offset = campaignTime.getTimezoneOffset() - tempDate.getTimezoneOffset();

  // Apply the offset to convert to UTC
  return new Date(dateInCampaignTZ.getTime() - (offset * 60000));
};
