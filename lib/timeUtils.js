...
export const convertScheduledTimeToUTC = (scheduledTime, campaignTimezone = 'UTC') => {
  ...
  try {
    // If scheduledTime is already an explicit UTC timestamp, keep it as-is.
    if (
      typeof scheduledTime === 'string' &&
      (scheduledTime.endsWith('Z') || /[+-]\d{2}:\d{2}$/.test(scheduledTime))
    ) {
      return new Date(scheduledTime);
    }

    // Treat incoming value as a wall-clock time in campaignTimezone.
    const localInput =
      scheduledTime instanceof Date
        ? format(scheduledTime, "yyyy-MM-dd'T'HH:mm:ss")
        : scheduledTime;

    if (campaignTimezone === 'UTC') {
      if (
        typeof localInput === 'string' &&
        !localInput.endsWith('Z') &&
        !/[+-]\d{2}:\d{2}$/.test(localInput)
      ) {
        return new Date(`${localInput}Z`);
      }
      return new Date(localInput);
    }

    return fromZonedTime(localInput, campaignTimezone);
  } catch (error) {
    console.error('Error converting scheduled time to UTC:', error);
    return null;
  }
};
