...
export class SequencerService {
  ...
  static async sendToProspect(campaignProspect) {
    ...
    if (!campaign || !prospect) {
      // Orphaned CampaignProspect — the campaign or prospect was deleted from DB
      // Stop this record so it doesn't keep appearing in ready-to-send queries
      console.warn(
        `⚠️ Orphaned CampaignProspect [${campaignProspect._id}]: ` +
        `campaign=${campaign ? campaign._id : 'NULL (deleted)'}, ` +
        `prospect=${prospect ? prospect._id : 'NULL (deleted)'}. ` +
        `Marking as stopped to prevent re-queuing.`
      );
      try {
        campaignProspect.status = 'stopped';
        campaignProspect.nextSendAt = null;
        ...
        console.error(`Failed to stop orphaned CampaignProspect ${campaignProspect._id}:`, e.message);
      }
      return;
    }
