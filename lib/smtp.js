...
export class SMTPService {
  static async sendEmail({ 
    ...
    try {
      ...
      // 4. Small, invisible tracking pixel at the very end (within body, not header)
      
      // Minimal tracking pixel - placed at end, invisible but trackable
      // Use a trick to prevent preview loads: wrapped in a div with minimal display
      // Gmail/Outlook only loads images when email is fully opened, not in preview pane
      const trackingPixel = `<div style="width:0;height:0;overflow:hidden;"><img src="${buildTrackingUrl(`/api/track/open/${trackingId}.gif`)}" width="0" height="0" border="0" style="display:none;"></div>`;
      
      // Wrap in minimal HTML (no fancy styling = personal email feel)
      // Avoid spam triggers: no excessive colors, no all-caps, no multiple exclamation marks
...
${trackingPixel}
      ...
      const mailOptions = {
        ...
        headers: {
          // Personal email headers (not marketing) - prevent spam folder
          'X-Priority': '3', // Normal priority (not urgent = not spam)
          'X-Mailer': 'Microsoft Outlook 16.0', // Looks like personal email
          'X-MimeOLE': 'Produced By Microsoft MimeOLE',
          'X-MSMail-Priority': 'Normal', // Not high priority = not spam
          'Importance': 'Normal',
          // NOTE: Removed 'Precedence: bulk' — it was breaking email threading
          // in Gmail/Outlook by signaling "mass email" which splits threads.
          // NOTE: Removed 'Auto-Submitted: no' — contradicts automated outreach
          // and some clients use it to suppress threading.
        },
