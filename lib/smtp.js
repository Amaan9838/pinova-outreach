...
export class SMTPService {
  static async sendEmail({ 
    ...
    try {
      ...
      // Clean tracking pixel — simple 1x1 image like trackmailbox.com and other
      // reputable services use. No hidden divs, no display:none, no width:0/height:0
      // tricks — those CSS hiding patterns are flagged by SpamAssassin.
      const trackingPixel = `<img src="${buildTrackingUrl(`/api/track/open/${trackingId}.gif`)}" width="1" height="1" alt="" style="border:0;">`;
      
      // Keep HTML simple — no DOCTYPE/head/body template wrapper.
      // Gmail web sends plain inline HTML, not a marketing email template.
      // The full template structure (DOCTYPE, meta viewport, max-width container)
      // signals bulk/marketing email and increases spam score.
